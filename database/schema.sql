...
-- ============================================================
-- STORED PROCEDURES
-- 2026-05-19: sincronizado con staging (faltaban en schema.sql)
-- ============================================================
...
CREATE PROCEDURE IF NOT EXISTS `sp_reporte_anual` (IN `p_anio` INT)
BEGIN
    SELECT '=== REPORTE ANUAL ===' as titulo, p_anio as anio;
...
BEGIN
    ...
    SELECT CONCAT('=== REPORTE MENSUAL: ', v_nombre_mes, ' ', p_anio, ' ===') as titulo;
    ...
    SELECT '=== DETALLE DE ÓRDENES ===' as detalle;
    ...
    LEFT JOIN (
        SELECT orden_id, SUM(precio_unitario * cantidad) as costo_refacciones
...
CREATE FUNCTION IF NOT EXISTS `FormatearTelefono` (`numero` VARCHAR(20))
RETURNS VARCHAR(20) CHARSET latin1 DETERMINISTIC READS SQL DATA
BEGIN
...
RETURNS LONGTEXT CHARSET utf8mb4 DETERMINISTIC READS SQL DATA
BEGIN
...
RETURNS TEXT CHARSET latin1 DETERMINISTIC READS SQL DATA
BEGIN
