-- =====================================================================
-- SEED: Fixture de prueba — Enero 2020 (Módulo Financiero)
--
...
--
-- Ejecutar: mysql -u user -p saggarag_staging < seed_test_enero_2020.sql
...
--
-- GET /api/financiero/gastos-admin?mes=1&anio=2020
--
...
-- utilidad_neta:       -11104.00   (3750-424-14200-230)
--
...
-- Capturar usuario admin para FKs
...
DELETE FROM gastos_administrativos
  WHERE mes = 1 AND anio = 2020 AND concepto LIKE '[SEED]%';
...
INSERT INTO clientes (nombre, telefono, notas) VALUES
  ('TEST-A Afinacion con IVA',   '5500000001', '[SEED] enero-2020'),
  ('TEST-B Frenos sin IVA',      '5500000002', '[SEED] enero-2020'),
  ('TEST-C Aceite con IVA',      '5500000003', '[SEED] enero-2020'),
  ('TEST-D Diagnostico sin IVA', '5500000004', '[SEED] enero-2020'),
  ('TEST-E Transmision ABIERTA', '5500000005', '[SEED] enero-2020');
...
INSERT INTO vehiculos (cliente_id, marca, modelo, anio, notas) VALUES
  (@c1, 'Nissan',     'Sentra',  2015, '[SEED] enero-2020'),
  (@c2, 'Toyota',     'Corolla', 2018, '[SEED] enero-2020'),
  (@c3, 'Volkswagen', 'Jetta',   2017, '[SEED] enero-2020'),
  (@c4, 'Chevrolet',  'Aveo',    2016, '[SEED] enero-2020'),
  (@c5, 'Ford',       'Focus',   2019, '[SEED] enero-2020');

SET @v1 = (SELECT id FROM vehiculos WHERE cliente_id = @c1 AND notas = '[SEED] enero-2020');
SET @v2 = (SELECT id FROM vehiculos WHERE cliente_id = @c2 AND notas = '[SEED] enero-2020');
SET @v3 = (SELECT id FROM vehiculos WHERE cliente_id = @c3 AND notas = '[SEED] enero-2020');
SET @v4 = (SELECT id FROM vehiculos WHERE cliente_id = @c4 AND notas = '[SEED] enero-2020');
SET @v5 = (SELECT id FROM vehiculos WHERE cliente_id = @c5 AND notas = '[SEED] enero-2020');
...
-- servicios=800 + mano_obra=400 = 1200, IVA=192, total=1392
INSERT INTO ordenes_servicio (
  ...
  'TEST-2020-001', @c1, @v1, @seed_user, 'Afinacion mayor programada',
  'cerrada', '2020-01-15 10:00:00',
  800.00, 400.00, 0.00,
...
-- mano_obra=500 + refacciones=1300, IVA=0, total=1800
-- Nota: 1300/1.30 = 1000.00 exacto — sin decimales
INSERT INTO ordenes_servicio (
  numero_orden, cliente_id, vehiculo_id, usuario_id, problema_reportado,
  estado, fecha_ingreso,
  subtotal_servicios, subtotal_mano_obra, subtotal_refacciones,
  ...
  'TEST-2020-002', @c2, @v2, @seed_user, 'Cambio de frenos delanteros',
  'cerrada', '2020-01-16 10:00:00',
  0.00, 500.00, 1300.00,
  0, 0.00, 1800.00, 0.00
);
...
-- servicios=500 + mano_obra=300 + refacciones=650 = 1450, IVA=232, total=1682
-- Nota: 650/1.30 = 500.00 exacto — sin decimales
INSERT INTO ordenes_servicio (
  numero_orden, cliente_id, vehiculo_id, usuario_id, problema_reportado,
  estado, fecha_ingreso,
  subtotal_servicios, subtotal_mano_obra, subtotal_refacciones,
  ...
  'TEST-2020-003', @c3, @v3, @seed_user, 'Cambio de aceite y filtro, revision de suspension',
  'entregada', '2020-01-18 10:00:00',
...
-- servicios=600 + mano_obra=200 = 800, IVA=0, total=800
INSERT INTO ordenes_servicio (
  numero_orden, cliente_id, vehiculo_id, usuario_id, problema_reportado,
  estado, fecha_ingreso,
  subtotal_servicios, subtotal_mano_obra, subtotal_refacciones,
  ...
  'TEST-2020-004', @c4, @v4, @seed_user, 'Diagnostico computarizado y balanceo',
  'cerrada', '2020-01-22 10:00:00',
  600.00, 200.00, 0.00,
  0, 0.00, 800.00, 0.00
);
SET @o4 = LAST_INSERT_ID();

-- Orden 5: ABIERTA — NO debe aparecer en ningún cálculo del balance
-- mano_obra=2000 + refacciones=1300, IVA=528, total=3828, costo_interno=200
-- Si aparece: total_facturado=9502, gastos_ordenes=430 → bug de filtrado confirmado
INSERT INTO ordenes_servicio (
  numero_orden, cliente_id, vehiculo_id, usuario_id, problema_reportado,
  estado, fecha_ingreso,
  subtotal_servicios, subtotal_mano_obra, subtotal_refacciones,
  ...
  'TEST-2020-005', @c5, @v5, @seed_user, 'Reparacion de transmision (en proceso)',
  'abierta', '2020-01-25 10:00:00',
...
-- ---------------------------------------------------------------
-- REFACCIONES_ORDEN
-- precio_unitario = costo de compra por unidad
-- subtotal        = precio_unitario * cantidad  (costo total, NO precio de venta)
-- El precio de venta va en ordenes_servicio.subtotal_refacciones (con 30% markup)
-- ---------------------------------------------------------------
-- Orden 2: sell=1300 → cost=1000 (1300/1.30), 1 unidad
INSERT INTO refacciones_orden (orden_id, descripcion, cantidad, precio_unitario, subtotal) VALUES
  (@o2, 'Pastillas de freno Brembo (juego)', 1.00, 1000.00, 1000.00);

-- Orden 3: sell=650 → cost=500 (650/1.30), 1 kit
INSERT INTO refacciones_orden (orden_id, descripcion, cantidad, precio_unitario, subtotal) VALUES
  (@o3, 'Filtro de aceite + aceite sintetico 4L (kit)', 1.00, 500.00, 500.00);

-- Orden 5 (abierta): sell=1300 → cost=1000
INSERT INTO refacciones_orden (orden_id, descripcion, cantidad, precio_unitario, subtotal) VALUES
  (@o5, 'Kit empaques transmision', 1.00, 1000.00, 1000.00);

-- ---------------------------------------------------------------
-- GASTOS ADMINISTRATIVOS — enero 2020
-- ---------------------------------------------------------------
INSERT INTO gastos_administrativos (mes, anio, concepto, monto, categoria, registrado_por) VALUES
  (1, 2020, '[SEED] Renta del local',     5000.00, 'renta',   @seed_user),
  (1, 2020, '[SEED] Salario mecanico',    8000.00, 'salario', @seed_user),
  (1, 2020, '[SEED] Agua, luz y telefono',1200.00, 'servicio',@seed_user);
...
SELECT
  ...
  SUM(total)                          AS sum_total_todas,
  SUM(CASE WHEN estado IN ('cerrada','entregada') THEN total ELSE 0 END) AS sum_total_validas
FROM ordenes_servicio
WHERE numero_orden LIKE 'TEST-2020-%';

SELECT
  SUM(monto) AS gastos_admin_enero_2020
...
DELETE FROM vehiculos WHERE notas = '[SEED] enero-2020';
DELETE FROM clientes  WHERE notas = '[SEED] enero-2020';
SELECT 'Teardown completo.' AS resultado;
*/
